...
(routes_agents.py) and per-workspace billing (routes_billing.py) all call
into this module instead of each
...
def authorize_agent_access(agent_id: str, *, agent_secret: Optional[str] = None,
    ...
    — used for read authorization (Task 4). Not used for claiming a
    brand-new agent_id (that's ensure_agent_secret's workspace_key
    precondition, a different operation: claiming vs. accessing).

    Both credentials are header-borne: the agent's own agent_secret, or the
    workspace_key of the workspace the agent belongs to (compared against the
    agent's workspace_id.txt). The third arm — a browser session cookie — was
    removed with Google sign-in (D-1162): with no login there is no cookie
    that could authorize a read, and no unauthenticated read path.
    """
